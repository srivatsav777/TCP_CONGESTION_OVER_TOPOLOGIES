...
pt = ggplot(df) + geom_line(aes(x="ts", y="cwnd" )) + geom_line(aes(x="ts", y="ssthresh" ), linetype='--') 

pt = pt + theme_bw() + geom_vline(aes(xintercept="ts" , color="sender") , data=lost , alpha=0.1 )

pt = pt + scale_y_continuous(name="CWND") + scale_x_continuous(breaks = list(range(0 , 40 , 2)) , name="Time (s)") + scale_colour_discrete()  
